classdef nozzle
   ...
   methods
       ...
       % Constructor
       function thisnozzle = nozzle(force, impulse, mass_flow_rate_exhaust, y, chamber_pressure, molar_mass_exhaust,chamber_temperature,altitude, wall_thickness,kind)
       ...
       function ctr = moc(thenozzle)
           ...
           % Diverging Angle
           theta_max = 0.5*v_PM(thenozzle.exit_mach)*RTOD; % Max Angle
           ...
           for m = 2:n+1
               thenozzle.ambient_temperature(m) = (delta_theta + (m-1))*thenozzle.dtor;
               x_int = [1 1.0001*thenozzle.exit_mach];
               func = @(x) thenozzle.ambient_temperature(m) - v_PM(x);
               M(m) = fzero(func,x_int);
               P(m) = 0 + thenozzle.throat_radius*tan(thenozzle.ambient_temperature(m)); %X-AXIS POINT
               RR(m) = -thenozzle.throat_radius/P(m);
               LR(m) = tan(thenozzle.ambient_temperature(m)+asin(1/M(m)));
               SL(m) = -RR(m);
           end
           ...
           for c = 1:length(P)-1
               ...
               y(c) = F*x(c)+thenozzle.throat_radius;
               X_P = [P(c) x(c)];
               Y_P = [0 y(c)];
               if plt ==1
                   plot(X_P,Y_P,'b');
